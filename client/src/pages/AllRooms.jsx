                ...
                {roomsDummyData.map((room) => (
                    <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0'>
                        <img onClick={() => { navigate(`/rooms/${room._id}`); scrollTo(0, 0) }}
                            src={room.images[0]} alt='hotel-img' title='View Room Details' className='max-h-65 md:w-1/2 rounded-xl shadow-lg objeact-cover object-cover cursor-pointer' />
                        <div className='md:w1/2 flex flex-col gap-2'>
                            ...
                            {/* Room Price */}
                            <p className='text-xl fint-medium text-gray-700'>${room.pricePerNight} /night</p>
                        </div>
                    </div>
                ))}
            </div>
